...
void ReadFromPipe(HWND hWnd, HANDLE g_hChildStd_OUT_Rd)
{
...
void MainWindow(HWND bodyWnd, HWND faceWnd)
{
	...
	HWND bodyBtn = CreateWindow(TEXT("button"), TEXT("Show Body"), WS_VISIBLE | WS_POPUP, 0, 0, 100, 50, bodyWnd, NULL, NULL, NULL);
	HWND faceBtn = CreateWindow(TEXT("button"), TEXT("Show Face"), WS_VISIBLE | WS_POPUP, 100, 0, 100, 50, faceWnd, NULL, NULL, NULL);

	ShowWindow(bodyBtn, SW_SHOW);
	UpdateWindow(bodyBtn);
	ShowWindow(faceBtn, SW_SHOW);
	UpdateWindow(faceBtn);
...
LRESULT CALLBACK WindowLabels(HWND hWnd, unsigned int uiMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uiMsg) {
	case WM_CLOSE:
		DestroyWindow(hWnd);
	...
	case WM_PAINT: {
		...
		char *cpaText[] = {
			"Happy: N/A  Angry: N/A ",
			"Closed: N/A  Neutral: N/A"
		};
		int iY = 5;
		for (int iLoopCounter = 0; cpaText[iLoopCounter] != '\0'; iLoopCounter++, iY += 20) {
			TextOut(hDC, 5, iY, (LPCWSTR)cpaText[iLoopCounter], strlen(cpaText[iLoopCounter]));
		}
		EndPaint(hWnd, &ps);
	}
				   break;
...
void labelWnd(int nCmdShow)
{
	...
	WindowClass.style = 0;
	WindowClass.lpszClassName = L"label";
	WindowClass.lpszMenuName = NULL;
	WindowClass.lpfnWndProc = WindowLabels;
	...
	if (initialised)
	{
		HWND labelWnd = CreateWindow(L"label", L"Labels", WS_OVERLAPPEDWINDOW, 200, 0, 100, 150, NULL, NULL, NULL, NULL);

		ShowWindow(labelWnd, nCmdShow);
...
int CBodyBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    // Create BodyBasics application window
